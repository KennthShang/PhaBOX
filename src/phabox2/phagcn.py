...
def run(inputs):
    ...
    if os.path.getsize(f'{rootpth}/{midfolder}/db_results.abc') == 0:
        ...
        df = pd.DataFrame({"Accession": Accession, "Length": Length_list,  "Lineage":['-']*len(Accession), "PhaGCNScore":[0]*len(Accession), "Genus": ['-']*len(Accession), "GenusCluster": ['-']*len(Accession)})
